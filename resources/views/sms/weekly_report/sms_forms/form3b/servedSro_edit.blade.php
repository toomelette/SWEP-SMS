...
@extends('layouts.modal-content',['form_id' => 'form3b_edit_servedSro_form_'.$rand,'slug'=>$servedSro->slug])
...
@section('scripts')
    <script type="text/javascript">
        $("#form3b_edit_servedSro_form_{{$rand}}").submit(function (e) {
            e.preventDefault();
            let form = $(this);
            loading_btn(form);
            $.ajax({
                url : '{{route("dashboard.form3b_servedSros.update",$servedSro->slug)}}',
                data : form.serialize(),
                type: 'PATCH',
                headers: {
                    {!! __html::token_header() !!}
