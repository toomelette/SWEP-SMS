...
class PlantillaSubscriber extends BaseSubscriber{
    ...
    public function onStore(){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:plantillas:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:plantillas:getAll');
    ...
    public function onUpdate($plantilla){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:plantillas:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:plantillas:findBySlug:'. $plantilla->slug .'');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:plantillas:getAll');
