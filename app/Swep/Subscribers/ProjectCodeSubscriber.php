...
class ProjectCodeSubscriber extends BaseSubscriber{
    ...
    public function onStore($project_code){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:project_codes:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:project_codes:getAll');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:project_codes:getByDepartmentName:'. $project_code->department_name .'');
    ...
    public function onUpdate($project_code){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:project_codes:getAll');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:project_codes:getByDepartmentName:'. $project_code->department_name .'');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:project_codes:findBySlug:'. $project_code->slug .'');
