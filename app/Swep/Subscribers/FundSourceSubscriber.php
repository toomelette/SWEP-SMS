...
class FundSourceSubscriber extends BaseSubscriber{
    ...
    public function onStore(){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:fund_sources:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:fund_sources:getAll');
    ...
    public function onUpdate($fund_source){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:fund_sources:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:fund_sources:getAll');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:fund_sources:findBySlug:'. $fund_source->slug .'');
