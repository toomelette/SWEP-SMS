...
class DisbursementVoucherSubscriber extends BaseSubscriber{
	...
	public function onStore($disbursement_voucher){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:disbursement_vouchers:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:disbursement_vouchers:fetchByUser:'. $disbursement_voucher->user_id .':*');
    ...
    public function onUpdate($disbursement_voucher){
        ...
        $this->__cache->deletePattern(''. config('app.name') .'_cache:disbursement_vouchers:fetch:*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:disbursement_vouchers:fetchByUser:'. $disbursement_voucher->user_id .':*');
        $this->__cache->deletePattern(''. config('app.name') .'_cache:disbursement_vouchers:findBySlug:'. $disbursement_voucher->slug .'');
